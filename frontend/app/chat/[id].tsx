...
export default function ChatScreen() {
  ...
  const micPressTimer = useRef<NodeJS.Timeout | null>(null);
  ...
  const handleMicPressIn = () => {
    // Start recording after a short delay to differentiate tap from hold
  ...
  const handleMicPressOut = () => {
    ...
    // If we're not recording yet (quick tap), do nothing
  };

  const handleRecordingComplete = async (audioUri: string, duration: number, waveform: number[]) => {
    ...
    try {
      ...
      const response = await api.post('/messages/voice', {
        receiver_id: id,
        audio_data: `data:audio/m4a;base64,${base64Audio}`,
        duration: duration,
        waveform: waveform,
      });

      // Message will arrive via WebSocket, but we can also add it locally
      ...
      <View>
        ...
        <View style={[styles.messageRow, isOwnMessage && styles.ownMessageRow]}>
          {item.message_type === 'voice' ? (
            ...
            <View style={[isOwnMessage && styles.ownMessageAlign]}>
              <VoiceMessageBubble
              ...
              />
              <View style={[styles.voiceMessageFooter, isOwnMessage && styles.ownVoiceFooter]}>
            ...
            >
              {item.message_type === 'image' ? (
                ...
                // Received message - show with translation options
                <TranslationBubble
                  messageId={item.id}
        ...
        {!isRecording && (
          <View style={[styles.inputContainer, { backgroundColor: theme.surface, borderTopColor: theme.border }]}>
            ...
            {inputText.trim() ? (
              ...
              <Pressable
                style={[styles.micButton, { backgroundColor: sendingVoice ? theme.textSecondary : '#FF3B30' }]}
